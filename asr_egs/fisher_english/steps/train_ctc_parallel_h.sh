...
##moar echos
     echo "$cuda_cmd JOB=1:$nj $dir/log/tr.iter$iter.JOB.log \
        $train_tool --report-step=$report_step --num-sequence=$num_sequence --frame-limit=$frame_num_limit \
        --learn-rate=$learn_rate --momentum=$momentum --num-jobs=$nj --job-id=JOB \
        --verbose=$verbose \
        ${utts_per_avg:+ --utts-per-avg=$utts_per_avg} \
        "$feats_tr" "$labels_tr" $dir/nnet/nnet.iter$[iter-1] $dir/nnet/nnet.iter${iter} >& $dir/log/tr.iter$iter.log"
...
##trying an echo to restart from here
      echo "$cuda_cmd JOB=1:$nj $dir/log/cv.iter$iter.JOB.log \
        $train_tool --report-step=$report_step --num-sequence=$valid_num_sequence --frame-limit=$frame_num_limit \
        --cross-validate=true --num-jobs=$nj --job-id=JOB \
        --learn-rate=$learn_rate \
        --verbose=$verbose \
        "$feats_cv" "$labels_cv" $dir/nnet/nnet.iter${iter} >& $dir/log/cv.iter$iter.log"
