...
if [ $stage -le 3 ]; then
  ...
  ##so there should be *many* more utterances with this now...

  ##split off (randomly) one third of the data to use since the full x3 set would take too long...
  utils/subset_data_dir_tr_cv.sh --cv-spk-percent 34 data/train_all data/train_rnd66 data/train_rnd34

  # Split the 'whole' training data into training (95%) and cross-validation (5%) sets
  utils/subset_data_dir_tr_cv.sh --cv-spk-percent 5 data/train_rnd34 data/train_tr95 data/train_cv05 || exit 1;

  #pick the first 720k utterances out of the training set (full set seems to cause memory errors, will need to investigate)
  utils/subset_data_dir.sh --first data/train_tr95 720000 data/train_720k;

  utils/subset_data_dir_tr_cv.sh --cv-spk-percent 20 data/train_720k data/train_tr80 data/test_deval;
...
#    data/train_cv05 data/train_cv05 $dir || exit 1;
  ...
  # Decoding with the basic vocabulary (from the CMU dict)
    steps/decode_ctc_lat.sh --cmd "$decode_cmd" --nj 10 --beam 17.0 --lattice_beam 8.0 --max-active 5000 --acwt 0.9 \
      data/lang_char_test data/test_dev93 $dir/decode_dev93 || exit 1;
    steps/decode_ctc_lat.sh --cmd "$decode_cmd" --nj 8 --beam 17.0 --lattice_beam 8.0 --max-active 5000 --acwt 0.9 \
      data/lang_char_test data/test_eval92 $dir/decode_eval92 || exit 1;
